# -*- coding: utf-8 -*-
"""部署 deb 到 iPhone（rootless 越狱 /var/jb）"""
...
print(); print("=" * 62); print("3. 刷新图标"); print("=" * 62)
so, se = sudosh("/usr/bin/killall -9 cfprefsd 2>/dev/null; "
                "/usr/bin/uicache -a 2>/dev/null; echo 刷新完成")
print("  " + (so + se).strip()[:150])

print(); print("=" * 62); print("4. 校验"); print("=" * 62)
so, se = sh("ls -d /var/jb/Applications/KSExtract.app 2>/dev/null || echo '未找到'")
print("  " + so.strip())

c.close()
print(); print("=" * 62)
print("安装完成。打开「快手上号器」测试。")
print("=" * 62)
