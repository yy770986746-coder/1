# -*- coding: utf-8 -*-
"""★ 完整反汇编 TestDecryptActivity 的所有方法"""
...
for cn, mn, co in methods:
    print("  %s  %s  @0x%x" % (cn.split("/")[-1].rstrip(";"), mn, co))

# 反汇编每个方法
OP = {
    0x0e:"return-void", 0x0f:"return", 0x10:"return-wide", 0x11:"return-object",
    0x0a:"move-result", 0x0c:"move-result-object", 0x01:"move", 0x04:"move-wide",
    0x07:"move-object", 0x00:"nop",
    ...
    0x4c:"aput-wide", 0x4d:"aput-object",
    ...
    0x90:"add-int", 0xb0:"add-int/2addr", 0xd8:"add-int/lit8",
}
...
def disasm(co, name):
    ...
    print()
    print("=" * 78)
    print("★ %s  @0x%x  (regs=%d ins=%d outs=%d insns=%d)" % (name, co, regs, ins, outs, insns))
    print("=" * 78)
    ip = co + 16
    end = ip + insns*2
    n = 0
    while ip < end and n < 200:
        op = data[ip]
        nm = OP.get(op)
        extra = ""
        size = 2
...
for cn, mn, co in methods:
    if co:
        disasm(co, "%s.%s" % (cn.split("/")[-1].rstrip(";"), mn))
